...
(function () {
    'use strict';
    asApp.controller('advisorGrantsController', ['$scope', '$rootScope', '$element', 'collegeServer', '$http', 'advisorServer', 'commonServer', 'NgTableParams', 'tools',
        function ($scope, $rootScope, $element, collegeServer, $http, advisorServer, commonServer, NgTableParams, tools) {
            $scope.vm = {
                tableFlag: 0,
                poorInfoList: {},
                feedbackList: {},
                    ...
                    if (data.status) {
                        ...
                        $scope.init = function () {
                            if (data.data.status === 5 && data.data.studentMaxStatus !== 7 && data.data.studentMaxStatus !== 8 && data.data.studentMaxStatus !== 101 || (data.data.status > 4 && (data.data.studentMaxStatus === 9 || data.data.studentMaxStatus === 102 || data.data.studentMaxStatus === 103))) {
                                 ...
                                 * 获取贫困等级列表
                                 */
                                advisorServer.getGrantsList().then(function (data) {
                                    if (data.status) {
                                        $scope.common(data);
                                    } else {
                                        $scope.vm.tableFlag = 2;
            ...
            $scope.common = function (data) {
                ...
                angular.forEach(data.data.dataList, function (data) {
                    data[0].selected = {
                        id: 0,
                        name: data[0].advisorAudit
                    };
                    if ($scope.vm.studentMaxStatus  === 10) {
                        data[0].advisorAnno = '正在进行';
                    }
             ...
             * 提交至学校审核
             */
            $scope.commitToCollegeCheck = function () {
                advisorServer.getListAfterCommitToCollegeCheck().then(function (data) {
                    if (data.status) {
